...
func main() {
	...
	mig := migrate.New(db,
		...
		migrate.WithMigrationTableName("migrations_2"),
	)
...
func getMigrations(fs embed.FS) (migrate.SqlMigrations, error) {
	...
	for i := 0; i < 4; i++ {
		...
		fd, err := fs.Open(fmt.Sprintf("migrations/0%d-down.sql", i+1))
		if err != nil {
			return nil, err
		}
		m, err := migrate.CreateSqlMigration(fmt.Sprintf("2%d", i+1), fu, fd)
		if err != nil {
			return nil, err
		}
		mig = append(mig, m)
